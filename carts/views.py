...
def add_cart(request, product_id): # 6
    ...
    if is_cart_item_exists:
        ...
        if quantity != None:
            cart_item = Cartitem.objects.create(cart=cart, quantity=float(quantity), product=product)
        else:
...
def remove_cart(request, product_id, cart_item_id):
    # CartItem
    try:
        cart_item = Cartitem.objects.get(cart__cart_id=_cart_id(request), product__id=product_id, id=cart_item_id)
